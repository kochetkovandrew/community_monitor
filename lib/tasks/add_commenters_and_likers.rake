task add_commenters_and_likers: :environment do
  Community.where(monitor_members: true).order(:id).each do |comm|
    ...
    topic_commenters = comm.topics.collect{|topic| (topic.post_comments || []).collect{|comment| comment.raw['from_id']}}.flatten.compact.sort.uniq
    ...
    all_users = (commenters + post_likes + comment_likes + topic_commenters).reject{|vk_id| vk_id<=0}.sort.uniq
