task :get_dialogs => :environment do |t, args|
  ...
  while (rest > 0) && cont_flag
    messages = vk_lock { vk.messages.get_history(peer_id: 2000000128, count: step_size, offset: step*step_size) }
    sleep 1
  ...
  fjson = File.open(Rails.root.join('applog', 'copycat.json'), 'w')
  fyaml = File.open(Rails.root.join('applog', 'copycat.yaml'), 'w')
